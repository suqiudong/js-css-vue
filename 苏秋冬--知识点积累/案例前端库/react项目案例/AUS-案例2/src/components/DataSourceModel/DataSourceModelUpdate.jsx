...
export default class DataSourceModelAdd extends Component {
    ...
    componentDidUpdate() {
        ...
        // 插入获取的数据源
        let dataSourceModelList = this.props.dataSourceModel.dataSourceModelList;
        if (dataSourceModelList || typeof (dataSourceModelList) === Array) {
                dataSourceModelList.map((dataName, i)=> {
                    // 防止页面重新渲染再次添加
                    if (deNodes.length <= i) {
                        deNodes.push({
        ...
        // 生成详情
        if (this.props.dataSourceModel.getModel.detail) {
            let detail = this.props.dataSourceModel.getModel.detail;
            this.refs.instance.getModels(detail);
            // 偏移字符
            this.OffsetField();
            let offsetColumn = detail.offsetColumn.split('.');
            $(' #offsetColumn').val(offsetColumn);

            // 添加input内容
            $(' #modelName').val(detail.modelName);
           // $(' #offsetColumn').append('<option value="date.modelName">' + date.modelName + '</option>');
            $(' #doneOffset').val(detail.doneOffset);
            $(' #description').val(detail.description);
            if (detail.scheduleStrategy == 'CYCLE') {
                $(' #endTime').css('display', 'block');
                $(' #cronDiv').css('display', 'block');
            }
            // 调度策略
            $(' #scheduleStrategy').val(detail.scheduleStrategy);
            // 调度状态
            $(' #schStatus').val(detail.schStatus);
            // 错过的调度状态
            $(' #misfireStrategy').val(detail.misfireStrategy);
            // 调度表达式
            if (detail.cronExpression) {
                let cronExpression = detail.cronExpression.split(' ');

                for (let i = 0; i < cronExpression.length; i++) {
                    if (cronExpression[i] > 0) {
                        $(' #cronExpression').val(cronExpression[i]);
                        $(' #TimeUnit').val(i);
                    }
                }
            }
            // 赋值sql
            tableModel.sql = detail.dataModelSQL;

            // 开始时间
            $(' #startTriggerTime').val(detail.startTriggerTime);

            // 结束时间
            $(' #endTriggerTime').val(detail.endTriggerTime);
        }
        // 生成tree数据结构
    ...
    saveModel(e) {
        ...
        dataSourceModelService.modifybasic(creatData);
        setTimeout(()=> {
                        ...
                        <form action="">
                            <div className='row'>
                                <div className='col-md-4 form-group'>
                                    ...
                                    <input type="text" id="modelName" maxLength={16} name="modelName" className="form-control" placeholder="请输入模型名称" disabled/>
                                </div>
                                <div className='col-md-4 form-group'>
